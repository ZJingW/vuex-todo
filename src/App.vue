<template>
  <div id="app">
    <a-input placeholder="请输入任务" class="my_ipt" :value="this.$store.state.inputValue"
    ...
    <a-list bordered :dataSource="this.$store.getters.infoList" class="dt_list">
      ...
      <div slot="footer" class="footer">
        <!-- 未完成的任务个数 -->
        <span>{{$store.getters.unDone}}条剩余</span>
        <!-- 操作按钮 -->
        <a-button-group>
          <a-button :type= "this.$store.state.str==='all'?'primary':'default'"
          
          @click="changeList('all')">全部</a-button>
          <a-button :type= "this.$store.state.str==='undone'?'primary':'default'"
           @click="changeList('undone')">未完成</a-button>
          <a-button :type= "this.$store.state.str==='done'?'primary':'default'"
    ...
    //监听复选框选中变化状态
     cbStatusChanged(e,id){
       //e.target.checked最新的选中状态
          ...
          this.$store.commit('checkStatus',param)
     },
     //清除已完成
     clean(){
       this.$store.commit('cleanDown')
     },
     //修改页面列表
     changeList(str){
     console.log(str)
